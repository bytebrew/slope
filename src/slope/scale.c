...
static void
_scale_add_item (SlopeScale *self, SlopeItem *item) {
    SlopeScalePrivate *priv = SLOPE_SCALE_GET_PRIVATE(self);
    if (item == NULL) {
        return;
    }
...
static void
_scale_remove_item (SlopeScale *self, SlopeItem *item) {
    SlopeScalePrivate *priv = SLOPE_SCALE_GET_PRIVATE(self);
    GList *iter;
...
static void
_scale_clear_item_list (gpointer data) {

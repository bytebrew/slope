...
SlopeScale *slope_xyscale_new_axis(
  ...
  slope_xyaxis_set_title(
      SLOPE_XYAXIS(priv->axis[SLOPE_XYSCALE_AXIS_BOTTOM]), x_title);
  slope_xyaxis_set_title(
      SLOPE_XYAXIS(priv->axis[SLOPE_XYSCALE_AXIS_LEFT]), y_title);
  slope_xyaxis_set_title(
      SLOPE_XYAXIS(priv->axis[SLOPE_XYSCALE_AXIS_TOP]), top_title);

  return SLOPE_SCALE(self);
}

static void _xyscale_draw(
    SlopeScale *self, const SlopeRect *rect, cairo_t *cr) {
  SlopeXyScalePrivate *priv = SLOPE_XYSCALE_GET_PRIVATE(self);
  int k;
  ...
  if (priv->on_drag == TRUE) {
    ...
    cairo_new_path(cr);
    cairo_rectangle(
        cr, priv->mouse_p1.x, priv->mouse_p1.y,
...
static void _xyscale_position_axis(SlopeScale *self) {
  ...
  slope_xyaxis_set_position(
      SLOPE_XYAXIS(priv->axis[SLOPE_XYSCALE_AXIS_BOTTOM]), priv->dat_x_min,
      priv->dat_x_max, priv->dat_y_min);
  slope_xyaxis_set_position(
      SLOPE_XYAXIS(priv->axis[SLOPE_XYSCALE_AXIS_TOP]), priv->dat_x_min,
      priv->dat_x_max, priv->dat_y_max);
  slope_xyaxis_set_position(
      SLOPE_XYAXIS(priv->axis[SLOPE_XYSCALE_AXIS_LEFT]), priv->dat_y_min,
      priv->dat_y_max, priv->dat_x_min);
  slope_xyaxis_set_position(
      SLOPE_XYAXIS(priv->axis[SLOPE_XYSCALE_AXIS_RIGHT]), priv->dat_y_min,
      priv->dat_y_max, priv->dat_x_max);
  slope_xyaxis_set_position(
      SLOPE_XYAXIS(priv->axis[SLOPE_XYSCALE_AXIS_X]), priv->dat_x_min,
      priv->dat_x_max, 0.0);
  slope_xyaxis_set_position(
      SLOPE_XYAXIS(priv->axis[SLOPE_XYSCALE_AXIS_Y]), priv->dat_y_min,
      priv->dat_y_max, 0.0);
}

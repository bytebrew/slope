...
static void _xyaxis_draw_horizontal(SlopeXyAxis *self, cairo_t *cr) {
  ...
  while (iter != NULL) {
    ...
    if (sample->label != NULL && (priv->component & SLOPE_XYAXIS_TICKS_DOWN ||
      ...
      slope_cairo_text(
          cr, sample_p1.x - txt_ext.width * 0.5,
  ...
  if (priv->title != NULL && (priv->component & SLOPE_XYAXIS_TITLE)) {
    ...
    if (priv->component & SLOPE_XYAXIS_TICKS_DOWN) {
      slope_cairo_text(
          cr, (p1.x + p2.x - txt_ext.width) / 2.0, p1.y + txt_height * 2.5,
          priv->title);
    } else if (priv->component & SLOPE_XYAXIS_TICKS_UP) {
      slope_cairo_text(
          cr, (p1.x + p2.x - txt_ext.width) / 2.0, p1.y - txt_height * 1.8,
          priv->title);
    } else {
      slope_cairo_text(
          cr, (p1.x + p2.x - txt_ext.width) / 2.0, p1.y + txt_height * 1.3,
          priv->title);
    }
  }
}
...
static void _xyaxis_draw_vertical(SlopeXyAxis *self, cairo_t *cr) {
  ...
  while (iter != NULL) {
    ...
    if (sample->label != NULL && (priv->component & SLOPE_XYAXIS_TICKS_DOWN ||
      ...
      slope_cairo_set_color(cr, priv->text_color);
      slope_cairo_text(
          cr, sample_p1.x + ((priv->component & SLOPE_XYAXIS_TICKS_DOWN)
                                 ? -txt_ext.width - txt_height * 0.3
                                 : +txt_height * 0.3),
          sample_p1.y + txt_height * 0.34, sample->label);
    }
  }
  ...
  if (priv->title != NULL && (priv->component & SLOPE_XYAXIS_TITLE)) {
    ...
    if (priv->component & SLOPE_XYAXIS_TICKS_DOWN) {
      slope_cairo_text(
          cr, -((p1.y + p2.y) + txt_ext.width) / 2.0,
          p1.x - max_txt_width - 1.0 * txt_height, priv->title);
    } else if (priv->component & SLOPE_XYAXIS_TICKS_UP) {
      slope_cairo_text(
          cr, -((p1.y + p2.y) + txt_ext.width) / 2.0,
          p1.x + max_txt_width + 1.6 * txt_height, priv->title);
    } else {
      slope_cairo_text(
          cr, -((p1.y + p2.y) + txt_ext.width) / 2.0, p1.x + 0.3 * txt_height,
          priv->title);
    }
    cairo_restore(cr);
  }

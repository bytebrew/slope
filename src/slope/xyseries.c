...
static void _xyseries_draw_thumb (SlopeItem *self, cairo_t *cr,
...
static void
_xyseries_draw (SlopeItem *self, cairo_t *cr) {
    SlopeXySeriesPrivate *priv = SLOPE_XYSERIES_GET_PRIVATE(self);
    if (priv->n_pts == 0L) {
        return;
    }
...
static void
_xyseries_draw_thumb (SlopeItem *self, cairo_t *cr, const SlopePoint *pos) {
    SlopeXySeriesPrivate *priv = SLOPE_XYSERIES_GET_PRIVATE(self);
    slope_cairo_set_antialias(cr, priv->antialias);
...
static void
_xyseries_draw_line (SlopeXySeries *self, cairo_t *cr) {
    SlopeXySeriesPrivate *priv = SLOPE_XYSERIES_GET_PRIVATE(self);
...
static void
_xyseries_draw_areaunder (SlopeXySeries *self, cairo_t *cr) {
    SlopeXySeriesPrivate *priv = SLOPE_XYSERIES_GET_PRIVATE(self);
    SlopeScale *scale = slope_item_get_scale(SLOPE_ITEM(self));
...
static void
_xyseries_draw_circles (SlopeXySeries *self, cairo_t *cr) {
    SlopeXySeriesPrivate *priv = SLOPE_XYSERIES_GET_PRIVATE(self);
    SlopeScale *scale = slope_item_get_scale(SLOPE_ITEM(self));
    ...
    for (k=0L; k<priv->n_pts; ++k) {
        ...
        slope_cairo_draw(cr, priv->symbol_stroke_color,
            priv->symbol_fill_color);
    }
}


static void
_xyseries_get_figure_rect (SlopeItem *self, SlopeRect *rect) {
    /* before we have a good "my rect procedure" use the scale's */
    slope_scale_get_figure_rect(slope_item_get_scale(self), rect);
}

static void
_xyseries_get_data_rect (SlopeItem *self, SlopeRect *rect) {
    SlopeXySeriesPrivate *priv = SLOPE_XYSERIES_GET_PRIVATE(self);
    rect->x = priv->x_min;
    rect->y = priv->y_min;

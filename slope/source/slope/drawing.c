...
void slope_cairo_set_color(cairo_t *cr, SlopeColor color) {
  cairo_set_source_rgba(
      cr, SLOPE_GET_REDF(color), SLOPE_GET_GREENF(color),
      SLOPE_GET_BLUEF(color), SLOPE_GET_ALPHAF(color));
}
...
void slope_cairo_line_cosmetic(
  ...
  if (__SIMILAR_DOUBLE(round_width, 1.0) ||
    ...
    cairo_line_to(cr, SLOPE_COORD_TO_PIXEL(p2->x), SLOPE_COORD_TO_PIXEL(p2->y));
      ...
      __SIMILAR_DOUBLE(round_width, 2.0) ||
...
void slope_cairo_round_rect(cairo_t *cr, const SlopeRect *rec, double radius) {
  ...
  cairo_arc(
      cr, rec->x + rec->width - radius, rec->y + radius, radius, -90 * degrees,
      0 * degrees);
  cairo_arc(
      cr, rec->x + rec->width - radius, rec->y + rec->height - radius, radius,
      0 * degrees, 90 * degrees);
  cairo_arc(
      cr, rec->x + radius, rec->y + rec->height - radius, radius, 90 * degrees,
      180 * degrees);
  cairo_arc(
      cr, rec->x + radius, rec->y + radius, radius, 180 * degrees,
      270 * degrees);
  cairo_close_path(cr);
}

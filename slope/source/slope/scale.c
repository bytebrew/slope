...
SlopeItem *slope_scale_get_item_by_name(
    SlopeScale *self, const char *itemname) {
  SlopeScalePrivate *priv = SLOPE_SCALE_GET_PRIVATE(self);
  GList *iter;
  iter = priv->item_list;
  while (iter != NULL) {
...
void _scale_draw_impl(SlopeScale *self, const SlopeRect *rect, cairo_t *cr) {
  ...
  if (priv->name != NULL && priv->show_name == TRUE) {
    ...
    slope_cairo_text(
        cr, rect->x + (rect->width - txt_ext.width) * 0.5,
        rect->y + txt_ext.height * 1.2 + priv->name_top_padding, priv->name);
    cairo_stroke(cr);
  }
}

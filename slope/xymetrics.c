...
_slope_xymetrics_get_class()
{
    ...
    if (first_call) {
        ...
        ((slope_object_class_t*) &klass)->init_fn = _slope_xymetrics_init;
...
_slope_xymetrics_init (slope_object_t *object)
{
    ...
    /* always call parent's init_fn */
    _slope_metrics_init(object);

    metrics = (slope_metrics_t*) object;
    metrics_priv = SLOPE_METRICS_GET_PRIVATE(object);
    priv = SLOPE_XYMETRICS_GET_PRIVATE(object);
    ...
    axis = slope_xyaxis_create(metrics, SLOPE_XYAXIS_TOP, "");
    priv->axis_list = slope_list_append(priv->axis_list, axis);

    axis = slope_xyaxis_create(metrics, SLOPE_XYAXIS_BOTTOM, "X");
    priv->axis_list = slope_list_append(priv->axis_list, axis);

    axis = slope_xyaxis_create(metrics, SLOPE_XYAXIS_LEFT, "Y");
    priv->axis_list = slope_list_append(priv->axis_list, axis);

    axis = slope_xyaxis_create(metrics, SLOPE_XYAXIS_RIGHT, "");
    priv->axis_list = slope_list_append(priv->axis_list, axis);
}
...
slope_xymetrics_create()
{
    ...
    _slope_xymetrics_init((slope_object_t*) self);
